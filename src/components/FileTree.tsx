...
function Item({ icon, name, active, badge, hasError, isLarge, onClick }: ItemProps) {
  return (
    <button className={`tree-item${active ? ' active' : ''}`} onClick={onClick} title={name}>
      ...
      {hasError && (
          <span className="parse-error-badge" style={{
            background: 'rgba(248,113,113,0.1)',
            color: 'var(--log-error)',
            border: '1px solid rgba(248,113,113,0.3)'
          }}><FaExclamation style={{marginRight: 6}}/> Error</span>
      )}
      {isLarge && (
        <span style={{
          display: 'inline-flex',
      ...
      {badge !== undefined && badge > 0 && (
        <span className="diff-badge"><FaCodeCompare style={{marginRight: 6}}/> {badge}</span>
      )}
    </button>
  );
}
