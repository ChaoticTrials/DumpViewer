  ...
  return (
    <div className="code-wrap" style={{ height: '100%' }}>
      <SyntaxHighlighter
        ...
        style={hlStyle}
        showLineNumbers
        customStyle={{ margin: 0, borderRadius: 0, fontSize: '12.5px', lineHeight: '1.6', height: '100%', padding: '8px 0' }}
  ...
  return (
    <>
      <div className="font-size-ctrl">
        <button className="font-size-btn" onClick={onDec} title="Decrease font size">−</button>
        <span className="font-size-val">{size}px</span>
        <button className="font-size-btn" onClick={onInc} title="Increase font size">+</button>
      </div>
      <button className="action-btn" onClick={onReset} title="Reset font size">↺</button>
    </>
  );
}
