...
export default function ManifestBanner({ manifest, onReset, onUpload, fileInputRef }: Props) {
  return (
    <header className="header">
      ...
      <div className="header-meta">
        ...
        {Object.entries(manifest.settings)
          .map(([key, enabled]) => enabled ? (
            ...
            <span key={key} className="badge" style={{ background: 'rgba(248,113,113,0.08)', color: '#f87171', borderColor: 'rgba(248,113,113,0.3)' }}>
              ✗ {SETTING_LABELS[key] ?? key}
            </span>
          ))}
      </div>
      ...
      <button className="upload-btn" onClick={onReset} style={{ background: 'rgba(255,255,255,0.05)', color: 'var(--text)', borderColor: 'var(--border)' }}>
        ✕ Close
      </button>
      <input
        ref={fileInputRef}
