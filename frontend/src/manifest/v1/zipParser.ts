...
export async function parseDump(file: File): Promise<ParsedDump> {
  ...
  const manifest: Manifest = JSON.parse(manifestText) as Manifest;
...
export function categorizeFiles(manifest: Manifest, files: Map<string, DumpFile>): CategorizedFiles {
  ...
  for (const entry of manifest.files) {
    ...
    if (entry.path.startsWith('config/')) {
      // v1: changed-values files use the same filename (json5 subset)
      const changedPath = `config/changed_values/${entry.name}`;
      const hasChanged = files.has(changedPath);
      configs.push({
        name: entry.name,
        fullPath: entry.path,
        changedPath: hasChanged ? changedPath : null,
        changedFormat: hasChanged ? 'json5' : null,
      });
